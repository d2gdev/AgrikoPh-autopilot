...
const review = z.object({ status: z.enum(["pending", "approved"]), approval: z.object({ identity: z.string().nullable(), approvedAt: z.string().datetime().nullable() }).strict(), validationImportEligible: z.boolean(), activationEligible: z.boolean(), operatorReviewRequired: z.literal(true), active: z.boolean(), approvalBasis: z.string(), approvalScope: z.string(), runtimeActivationAuthorized: z.boolean(), liveExecutionAuthorized: z.boolean(), canonicalIndexationExecutionProhibited: z.literal(true), task3Authorized: z.literal(false) }).strict().superRefine((v, ctx) => {
  if (v.status === "approved" && (!v.approval.identity || !v.approval.approvedAt || !v.validationImportEligible)) ctx.addIssue({ code: z.ZodIssueCode.custom, message: "approval" });
  if (v.status === "pending" && (v.validationImportEligible || v.activationEligible || v.runtimeActivationAuthorized)) ctx.addIssue({ code: z.ZodIssueCode.custom, message: "pending" });
  if (v.activationEligible !== v.runtimeActivationAuthorized) ctx.addIssue({ code: z.ZodIssueCode.custom, message: "activation authorization" });
  if (v.active) ctx.addIssue({ code: z.ZodIssueCode.custom, message: "active" });
  if (v.liveExecutionAuthorized) ctx.addIssue({ code: z.ZodIssueCode.custom, message: "live execution" });
});
