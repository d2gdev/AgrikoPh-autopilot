...
45 6 * * * root SECRET=$(grep '^CRON_SECRET=' /opt/autopilot/.env | cut -d= -f2 | tr -d '"'); curl -sf https://${DOMAIN}/api/cron/reindex-published -H "Authorization: Bearer $SECRET" >> /var/log/autopilot-cron.log 2>&1
