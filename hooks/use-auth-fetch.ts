...
async function primeAppBridgeAuthState(
  ...
  while (Date.now() - startedAt <= timeoutMs) {
    ...
    if (!win) {
      setAuthSnapshot({
        ...
        appBridgeReady: false,
        initialized: false,
        error: "Shopify App Bridge unavailable outside the browser",
      });
      return;
    }
    ...
    if (isTopLevelWindow()) {
      setAuthSnapshot({
        ...
        initialized: false,
        error: "Shopify App Bridge unavailable outside Shopify Admin",
      });
      return;
    }

    if (!context.hasHost && !win.shopify) {
      setAuthSnapshot({
        status: "error",
        hasHost: false,
        appBridgeReady: false,
    ...
    const hasShopify = Boolean(win.shopify);
    if (!bootLogged) {
      bootLogged = true;
      logAuth("info", "Shopify context checked", {
        ...
        appBridgeReady: hasShopify,
      });
    }

    setAuthSnapshot({
      status: hasShopify ? "ready" : "loading",
      hasHost: context.hasHost,
      appBridgeReady: hasShopify,
      initialized: hasShopify,
      error: null,
    });
    ...
    if (win.shopify) {
      win.shopify.ready
        ...
        .catch((err) => {
          logAuth("warn", "Shopify App Bridge ready promise rejected", {
  ...
  setAuthSnapshot({
    ...
    appBridgeReady: false,
    initialized: false,
    error: `Shopify App Bridge unavailable after ${timeoutMs}ms`,
  });
}
