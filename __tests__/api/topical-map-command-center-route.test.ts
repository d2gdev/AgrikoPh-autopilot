...
describe("GET /api/topical-map/command-center", () => {
  ...
  it("projects every domain from one bounded active-version query without raw artifacts", async () => {
    ...
    expect(db.topicalMapActivation.findUnique).toHaveBeenCalledWith({
      ...
      select: {
        strategyVersion: { select: {
          id: true, strategyVersion: true, contractRevision: true, packageSha256: true, activatedAt: true,
