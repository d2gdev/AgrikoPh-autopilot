...
describe.skipIf(!url)("PostgreSQL topical-map activation", () => {
  ...
  it("serializes concurrent activation, supersedes exactly one prior active version, and keeps pointer aligned", async () => {
    ...
    expect(pointer.strategyVersionId).toBe(active[0]!.id);
    expect(await prisma.topicalMapStrategyVersion.findUniqueOrThrow({ where: { id: old.id } })).toMatchObject({ lifecycle: "superseded" });
    const audit = await prisma.auditLog.findMany({ where: { action: "topical_map_strategy_activated", entityType: "topical_map_strategy" }, orderBy: { createdAt: "desc" }, take: 1 });
    expect(audit[0]).toMatchObject({ meta: expect.objectContaining({ siteHost: host }), before: expect.objectContaining({ versionId: old.id, packageSha256: old.packageSha256 }), after: expect.objectContaining({ versionId: active[0]!.id, packageSha256: active[0]!.packageSha256 }) });
  }, 30000);

  it("rolls back only to a historically validated same-site version atomically with provenance", async () => {
    const current = await prisma.topicalMapStrategyVersion.findFirstOrThrow({ where: { siteHost: host, lifecycle: "active" } });
    const historic = await prisma.topicalMapStrategyVersion.findFirstOrThrow({ where: { siteHost: host, lifecycle: "superseded", validationStatus: "valid" } });
