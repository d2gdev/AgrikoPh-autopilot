...
- Recommendation outcome feedback loop: `jobs/check-outcomes.ts` compares before/after connector snapshots for `executed` recommendations 7+ days old, writes the verdict to `Recommendation.outcome`/`outcomeCheckedAt`, and indexes a `recommendation_outcome` KB chunk so future skill runs are grounded in what actually worked. **Deploy ordering is a hard requirement**: migration `20260702100000_add_recommendation_outcome` MUST be applied (`npm run db:migrate` on prod) BEFORE deploying this code — the regenerated Prisma client selects the new columns on every `Recommendation` read, so deploying the code first breaks the daily pipeline and the approvals UI with a Prisma `P2022` (column does not exist) error. New external-scheduler entry: `/api/cron/check-outcomes` (see `docs/CRON.md`). `jobs/index-knowledge.ts`'s orphan-deletion pass is scoped to `INDEXED_SOURCE_TYPES` (`lib/ai/knowledge-sources.ts`) so it never deletes `recommendation_outcome` chunks, which it doesn't own.
