...
- **A route file may only export route handlers + config** (`GET`/`POST`/`dynamic`/`maxDuration`/…). Exporting any other symbol (e.g. a pure helper you want to unit-test) fails `tsc` with `OmitWithTag … not assignable to never`. Put testable helpers in a `lib/**` module and import them into the route.
- **Normalize priority before sorting/toning across sources.** Priority values are inconsistent across tables: `StoreTask`/`Opportunity`/most `ContentProposal` use `P0–P3`, but some `ContentProposal` rows use word grades (`High`/`Medium`/`Low`). A raw string comparator (`a.priority > b.priority`) sorts `"Medium"` before `"P1"` and never returns 0 for ties. Use `lib/growth-brief/priority.ts` `priorityRank()` (or an equivalent numeric map) and sort by rank difference.
