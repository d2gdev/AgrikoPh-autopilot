...
22. Overview article rows must load and render one bounded server page. Return authoritative full-corpus summary counts separately from that page, retain exact `(blogHandle, handle)` identity for links, prevent older overlapping refreshes from overwriting newer state, and guard the initial tab transition from issuing duplicate reads.
...
- A unique database key does not guarantee a unique operator decision. If dozens of distinct ad IDs render the same competitor and message, group them in presentation and keep their individual evidence links accessible.
