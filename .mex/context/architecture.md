...
- **`lib/content-pilot/`** — separate pipeline for blog content: governed Brief/SEO Pilot work creates `ContentProposal` rows → operator approves → draft generation via AI → publish to Shopify blog. Governed drafts with an exact blog URL resolve the Shopify source by `(blogHandle, handle)` before AI generation and fail closed if that exact source is unavailable; handle-only fallback is limited to legacy proposals without an exact URL. Internal-link prose may be model-assisted, but the single exact persisted anchor is constructed deterministically and validated before persistence. The operator Queue is review-only and uses server-side filters, authoritative aggregate stage counts, bounded 25-row pages, and collapsed evidence details rather than downloading or fully expanding terminal history in the browser. Content Pilot mounts only its selected tab and guards the initial Overview transition from duplicate reads. Overview renders one 25-article page while its API returns authoritative full-corpus SEO counts; article diagnostics retain exact blog identity for storefront links. Topic and inbound-link panels are descriptive observations derived from indexed articles, never independent content-gap, priority, cluster-role, or pillar recommendations.
