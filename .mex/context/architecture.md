...
- **Rolling topical-map SEO task scheduling** — `lib/seo-tasks/topical-map-scheduler.ts` projects only resolved typed schedule obligations whose complete schedule authority boundary remains proposal-only and execution-prohibited. It groups strict `Days N-M: Label` headings into phase-level `SeoFollowUpTask` records, anchors them to the active version's Manila activation date, applies only strict source-authored `On or after YYYY-MM-DD` review-date overrides, and materializes phases whose review date is within 90 days. Immutable strategy-version/phase source keys reuse the existing task dedupe and terminal-history protections. Activation and rollback trigger a best-effort immediate refresh after their atomic lifecycle transaction; the already locked daily route is the durable retry and alert path. This scheduler creates local review records only and grants no Shopify, Meta, recommendation-execution, or elapsed-time mutation authority.
