...
- **Topical-map Store Task execution** — after an operator-triggered SEO analysis successfully persists its exact active-map snapshot, it opportunistically synchronizes non-blog Store Tasks and returns bounded counts; sync failure is isolated from analysis readiness, and the authenticated standalone sync route is the retry boundary. Blog articles remain governed by Content Pilot. Executable tasks are limited to product, collection, and page SEO metadata, additive content, and internal links. Apply requires explicit operator confirmation, reloads the exact active strategy and live Shopify before-state, rejects strategy/state drift, and persists the Shopify receipt before marking completion. Homepage, blog index, redirects, canonicalization, indexation, and unavailable drafts remain advisory-only; no synchronization path autonomously applies a task.
