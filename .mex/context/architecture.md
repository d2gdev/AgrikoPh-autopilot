...
- **Topical-map Store Task execution** — synchronization creates/links a pending Shopify Recommendation for each executable non-blog Store Task. Store Pilot Apply loads bounded exact detail and only approves/queues that Recommendation. `execute-approved` is the sole live dispatcher and requires both approved status and `EXECUTE_APPROVED_LIVE_ENABLED=true`; it revalidates strategy/rules/current Shopify state, serializes by normalized target with a persisted lock, and completes both records only after a verified receipt. Failed or uncertain writes remain failed and require re-sync/reobservation. Homepage, blog index, redirects, canonicalization, indexation, and unavailable/oversized drafts remain advisory-only.
