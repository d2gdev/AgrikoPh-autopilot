...
last_updated: 2026-07-13T22:09:00+08:00
---
...
- **Topical-map Store Task execution** — synchronization creates/links a pending Shopify Recommendation for each executable non-blog Store Task and freezes approved bytes from later sync. Store Pilot Apply loads bounded exact detail and only approves/queues that Recommendation with the strict proposed hash. `execute-approved` is the sole live dispatcher and requires both approved status and `EXECUTE_APPROVED_LIVE_ENABLED=true`; it compares the approved hash before access, revalidates strategy/rules/current Shopify state, and serializes by normalized target with an owned expiring lock. The service returns only a minimal verified receipt; one local transaction completes Store Task + Recommendation, writes both audits/receipt, and releases the lock. Applying/reconciliation/failed states remain visible, stale execution is reobserved, and retry is re-sync rather than replay.
