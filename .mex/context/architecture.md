...
- **SEO Pilot strategy-bound command center** — `GET /api/topical-map/command-center` authenticates before its bounded active-pointer read and projects identity, all eleven rule-domain counts, ownership/pages (including the compiled title separately from the target keyword), governed work, gates/review requirements, and source references without raw strategy artifacts. Revision-3 P0/P1/P2/P3 priorities normalize to operator high/medium/low bands without changing source badges or persisted values. Article evidence uses exact `(blogHandle, handle)` identity and normalized blog paths. SEO analysis snapshots are versioned envelopes bound to the exact strategy version/package hash and carry deterministic candidate IDs. Selected promotion accepts only the exact analysis timestamp/identity plus at most 100 IDs, reloads current server evidence, derives the proposal title from the active map, and commits each candidate independently through governed persistence. Internal-link presentation assigns an operational label only from an actual candidate, non-blog routing, or persisted suppression evidence; absence of per-rule evidence stays neutral. Unmapped GSC demand stays observational; redirect state comes from Store Pilot observation, while canonical/indexation remain advisory and expose their bounded map instructions.
- **Topical-map Store Task execution** — synchronization creates/links a pending Shopify Recommendation only when a non-blog content decision explicitly and unambiguously requests SEO metadata or body-content work; keep/preserve/indexation/publication/conditional/ambiguous decisions fail closed. It freezes approved bytes from later sync and gives advisory work stable semantic identity with history-preserving supersession; retained pending/failed canonical/index advisories refresh bounded display detail on that identity. Shopify resource observations separate local capture time from resource update time and process bounded AI draft requests in deterministic 25-item chunks. Redirect support is create-only for a verified absent exact source, with a strict absent→target list/detail DTO; matching redirects are satisfied and conflicts remain advisory. Store Pilot approval and execution remain separate operator actions. Live dispatch still requires approved status and `EXECUTE_APPROVED_LIVE_ENABLED=true`, then revalidates proposed hash, strategy, rules, current Shopify state, and the owned target lock. Stale identity/state conflicts become audited superseded work, while actual Shopify uncertainty remains bounded reconciliation work. A verified receipt completes Store Task and Recommendation atomically.
