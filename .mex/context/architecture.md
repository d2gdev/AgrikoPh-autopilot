...
last_updated: 2026-07-14T08:32:34+08:00
---
...
- **SEO Pilot strategy-bound command center** — `GET /api/topical-map/command-center` authenticates before its bounded active-pointer read and projects identity, all eleven rule-domain counts, ownership/pages, governed work, blockers, and source references without raw strategy artifacts. SEO analysis snapshots are versioned envelopes bound to the exact strategy version and package hash; readers withhold mismatched cached analysis as stale. Promotion accepts only candidates reconstructed from the current projection and revalidates the same identity/rules inside governed proposal persistence. Unmapped GSC demand stays observational, redirect persistence is unsupported, and canonical/indexation remain advisory with no live-execution authority.
- **Topical-map Store Task execution** — synchronization creates/links a pending Shopify Recommendation for each executable non-blog Store Task, freezes approved bytes from later sync, and gives advisory work stable semantic identity with history-preserving supersession. Store Pilot queries one bounded actionable or advisory page at a time with database-backed totals; approval and execution are separate operator actions. The execution endpoint accepts no proposed bytes and delegates only the persisted linked Recommendation to `execute-approved`; scheduled batches remain unchanged. Live dispatch still requires approved status and `EXECUTE_APPROVED_LIVE_ENABLED=true`, then revalidates proposed hash, strategy, rules, current Shopify state, and the owned target lock. Stale identity/state conflicts become audited superseded work, while actual Shopify uncertainty remains bounded reconciliation work. A verified receipt completes Store Task and Recommendation atomically, and the outcome job later compares exact URL-level seven-day GSC windows or records a typed insufficient-data result.
