...
Topical-map evaluation is a separate server-only evidence boundary. It never executes redirect, canonical, or indexation work; every result remains `executionAuthorized: false`. Explicit structured medical, dosage, safety, and health classifications require `manual_high_stakes_review`. Never infer governed URLs, ownership, evidence, or high-stakes status from AI prose.

Topical-map package validation and import are operator-triggered, not a skill or
cron workflow: no topical-map validation cron exists. Import/validation never
activates a strategy and never writes to Shopify or Meta. The approved July 12
six-artifact package (contract revision 2) is validation/import-eligible only;
its activation eligibility and runtime activation authorization remain false.
